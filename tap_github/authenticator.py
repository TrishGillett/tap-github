...
class TokenManager:
    ...
    def __init__(self, token, rate_limit_buffer=None, logger=None):
        self.token = token
        self.logger = logger
...
class AppTokenManager(TokenManager):
    ...
    def refresh_token(self):
        if self.github_private_key:
            ...
            if is_valid:
                self.token = token
                self.token_expires_at = token_produced_at + timedelta(hours=1)
            else:
    ...
    def has_calls_remaining(self) -> bool:
        ...
        has_calls_remaining = self.rate_limit_used > (self.rate_limit - self.rate_limit_buffer)
        has_time_remaining = datetime.now() + timedelta(minutes=self.expiry_time_buffer_mins) < self.token_expires_at
...
class GitHubTokenAuthenticator(APIAuthenticatorBase):
    ...
    def prepare_tokens(self) -> List[TokenManager]:
        ...
        self.logger.info(f"Tap will run with {len(token_list)} auth tokens")
        return token_list
    ...
    def __init__(self, stream: RESTStream) -> None:
        ...
        self.active_token: Optional[TokenManager] = (
            choice(self.token_list) if self.token_list else None
        )
